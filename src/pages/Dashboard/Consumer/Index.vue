<template>
<div class="q-pa-md doc-container">
    <q-badge>Products Available</q-badge>
     <q-input
        v-model="search"
        debounce="1000"
        filled
        placeholder="Search"
        hint="Search for Product"
    style="margin-top:25px"
      >
        <template v-slot:append>
          <q-icon name="search" />
        </template>
      </q-input>
    <div class="row items-start">
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer1.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            Agrovit - 500gm
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹140.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer2.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            Humacid - 500ml
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹360.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer3.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            MahaFlora - 1 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer4.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            MahaGreen - 1 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>

        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer5.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            Cow Manure - 5 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer6.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            Vermicompost - 5 kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer3.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            MahaFlora - 1 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer3.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            MahaFlora - 1 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
        <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card">
                <q-img src="~assets/images/fertilizer4.jpg" />

                <q-card-section>
                    <q-btn fab color="primary" icon="share" class="absolute" style="top: 0; right: 12px; transform: translateY(-50%);" />

                    <div class="row no-wrap items-center">
                        <div class="col text-h6 ellipsis">
                            MahaGreen - 1 Kg
                        </div>
                        <div class="col-auto text-grey text-caption q-pt-md row no-wrap items-center">
                            <q-icon name="place" />
                            Plant Fertilizer
                        </div>
                    </div>

                    <q-rating v-model="stars" :max="5" size="32px" />
                </q-card-section>

                <q-card-section class="q-pt-none">
                    <div class="text-caption" secoundary>
                        In Stock (13pc)
                        Price: ₹315.00
                    </div>
                    <div class="text-caption text-grey">
                        It is safe and an excellent wetting agent which improves<br /> wetting,spreading, sticking quality and enhance<br /> effectiveness of pesticidant spray.
                    </div>
                </q-card-section>
                <q-separator />
                <q-card-actions>
                    <q-btn flat icon="shopping_cart" color="primary">
                        &nbsp; &nbsp;Add to Cart
                    </q-btn>
                    <q-btn flat icon="add_shopping_cart" color="primary">
                        &nbsp; &nbsp;Buy Now
                    </q-btn>
                </q-card-actions>
            </q-card>
        </div>
    </div>
</div>
</template>

<script>
export default {

}
</script>

<style>
    
    </style>
